...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{musica(interfaccia,senso)}};
  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{che cos'è musica?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=musica, very thick, ->] (-5,0) -- (5,0);
     \draw[upper arc=interfaccia, very thick, ->] (-3,0) arc (180:2:3cm);

     \draw[lower arc=senso, very thick, ->] (3,0) arc (0:-178:3cm);
...
%    \node[below=3pt, font=\Large] at (0,-3) {IIR};
    ...
    \node[anchor= east,font=\Large, rotate=3] at (-5,0) {$suono$};
    \node[anchor= west,font=\Large, rotate=3] at (5,0) {$uomo$};
  \end{scope}
