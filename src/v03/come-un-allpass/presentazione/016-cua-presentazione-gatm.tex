...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{ascolto(udire,sentire)}};
  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{qual è il processo che dà senso all'opera?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=ascolto, very thick, ->] (-5,0) -- (5,0);
     \draw[upper arc=udire, very thick, ->] (-3,0) arc (180:2:3cm);

     \draw[lower arc=sentire, very thick, ->] (3,0) arc (0:-178:3cm);
