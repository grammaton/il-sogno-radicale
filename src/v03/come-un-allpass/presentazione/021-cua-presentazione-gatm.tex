...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{aristosseno(accadere,accaduto)}};
%  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{che cos'è un'interprete?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=aristosseno, very thick, ->] (-7,0) -- (7,0);
     \draw[upper arc=accadere, very thick, ->] (-3,0) arc (180:2:3cm);
     \draw[lower arc=accaduto, very thick, ->] (3,0) arc (0:-178:3cm);
