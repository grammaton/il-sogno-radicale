...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{opera(prassi,teoria)}};
  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{che cos'è un'opera?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=opera, very thick, ->] (-7,0) -- (7,0);
     \draw[upper arc=prassi, very thick, ->] (-3,0) arc (180:2:3cm);
     \draw[lower arc=teoria, very thick, ->] (3,0) arc (0:-178:3cm);

     \draw[upper arc=strumento, very thick, ->] (-4,0) arc (180:2:4cm);
     \draw[lower arc=interprete, very thick, ->] (4,0) arc (0:-178:4cm);

     \draw[upper arc=scrittura, very thick, ->] (-5,0) arc (180:2:5cm);
     \draw[lower arc=ascolto, very thick, ->] (5,0) arc (0:-178:5cm);
