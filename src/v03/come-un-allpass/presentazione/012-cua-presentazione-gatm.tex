...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{esperienza(materia,memoria)}};
  \node[anchor=south west, font=\Huge] at (-13,-7) {\emph{che cos'è un'esperienza?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=esperienza, very thick, ->] (-5,0) -- (5,0);
     \draw[upper arc=materia, very thick, ->] (-3,0) arc (180:2:3cm);

     \draw[lower arc=memoria, very thick, ->] (3,0) arc (0:-178:3cm);
