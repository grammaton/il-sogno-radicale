...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{esperienza-musicale(opera,ascolto)}};
  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{che cos'è un'esperienza musicale?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=esperienza musicale, very thick, ->] (-5,0) -- (5,0);
     \draw[upper arc=opera, very thick, ->] (-3,0) arc (180:2:3cm);

     \draw[lower arc=ascolto, very thick, ->] (3,0) arc (0:-178:3cm);
