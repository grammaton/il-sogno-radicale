...
%  \node[anchor=north west, font=\Huge] at (-13,7) {$y[n] = x[n-m] - g \cdot x[n]$};
  \node[anchor=north west, font=\Huge] at (-13,6) {\texttt{circuito(segnale,processo)}};
  \node[anchor=west, font=\Huge] at (-13,-6.5) {\emph{che cos'è un circuito?}};  
  
  \begin{scope}[rotate=3]
     \draw[base line=circuito, very thick, ->] (-5,0) -- (5,0);
     \draw[upper arc=segnale, very thick, ->] (-3,0) arc (180:2:3cm);

     \draw[lower arc=processo, very thick, ->] (3,0) arc (0:-178:3cm);
